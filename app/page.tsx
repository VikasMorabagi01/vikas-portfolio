            ...
            <section id="home" className="min-h-[85vh] flex flex-col justify-center items-center text-center relative pt-10">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex gap-4 mt-8 z-10 items-center">
                ...
                <a 
                  ...
                  className="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold tracking-wide rounded-none border border-emerald-400 shadow-[4px_4px_0_#34d399] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_#34d399] transition-all flex items-center gap-2"
                >
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {data.skills.map((skillGrp, i) => (
                  <motion.div 
                    ...
                    className="glass-panel rounded-2xl p-6 border border-slate-800 hover:border-emerald-500/30 transition-colors"
                  >
                    <h4 className="text-emerald-400 font-mono mb-4 text-sm uppercase tracking-widest flex items-center gap-2">
                      <span className="text-slate-600">{'//'}</span> {skillGrp.category}
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {skillGrp.items.map((item, j) => (
                        <span key={j} className="px-3 py-1.5 bg-slate-900 border border-slate-700 hover:border-emerald-500/50 hover:text-emerald-300 rounded text-sm text-slate-300 font-medium transition-colors cursor-default">
                          {item}
                        </span>
                      ))}
                    </div>
                ...
                <div>
                  ...
                  <div className="space-y-6">
                    {data.education.map((edu, i) => (
                      <div key={i} className="p-6 glass-panel rounded-xl border border-slate-800 hover:border-teal-500/30 transition-all">
                        <h4 className="text-lg font-bold text-white mb-1">{edu.degree}</h4>
                        <p className="text-slate-400 text-sm mb-4">{edu.institution}</p>
                        <div className="flex justify-between items-center text-xs font-mono">
                          <span className="text-teal-500">{edu.dates}</span>
                          <span className="bg-teal-500/10 text-teal-300 px-2 py-1 rounded">{edu.details}</span>
                        </div>
                      </div>
                    ))}
                  </div>
